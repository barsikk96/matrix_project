...
void test_compute_expression() {
    ...
    // temp4 = temp3 * D = [5*13 + 7*15, 5*14 + 7*16; 6*13 + 8*15, 6*14 + 8*16] = [170, 182; 198, 212]
    // result = temp4 = [170, 182; 198, 212]
    ...
    for (int i = 0; i < result->rows; i++) {
        for (int j = 0; j < result->cols; j++) {
            assert(result->data[i][j] == expected->data[i][j] && "Ошибка: результат не совпадает с ожидаемым значением\n");
        }
    }
