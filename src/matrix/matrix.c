...
Matrix *create_matrix(int rows, int cols) {
  ...
  if (rows > 0 && cols > 0) { ///< Проверка корректности размеров матрицы
    char flag_error = 0;
    matrix = (Matrix *)malloc(
        sizeof(Matrix)); ///< Выделение памяти для структуры Matrix
    if (matrix) {
      matrix->rows = rows;
      matrix->cols = cols;
      matrix->data = (MATRIX_TYPE **)calloc(
          rows, rows * sizeof(MATRIX_TYPE *)); ///< Выделение памяти для массива
      ...
      if (matrix->data) { ///< Проверка успешности выделения памяти под строки
        for (size_t i = 0; i < (size_t)rows && flag_error == 0; i++) {
          matrix->data[i] = (MATRIX_TYPE *)calloc(
              cols, cols * sizeof(MATRIX_TYPE)); ///< Выделение памяти под
                                                 ///< столбцы каждой строки
          if (!matrix->data[i]) { ///< Проверка успешности выделения памяти
            while (i-- > 0) {
              free(matrix->data[i]); ///< Освобождение уже выделенной памяти в
                                     ///< случае ошибки
            }
            free(matrix->data);
            free(matrix);
        ...
        free(matrix); ///< Освобождение памяти в случае ошибки выделения под
                      ///< массив строк
        matrix = NULL;
      }
    }
  ...
  return matrix; ///< Возвращение указателя на созданную матрицу или NULL в
                 ///< случае ошибки
}
...
Matrix *add_matrices(const Matrix *A, const Matrix *B) {
  ...
  if (A && B && A->rows == B->rows &&
      A->cols == B->cols) { // Проверка корректности
    ...
    if (result) { ///< Проверка успешного выделения памяти
      for (size_t i = 0; i < (size_t)A->rows; i++) {
        for (size_t j = 0; j < (size_t)A->cols; j++) {
          result->data[i][j] =
              A->data[i][j] + B->data[i][j]; ///< Сложение элементов
        }
      }
    }
  }
  ...
  if (A && B && A->rows == B->rows &&
    ...
    if (result) { ///< Проверка успешного выделения памяти
      for (size_t i = 0; i < (size_t)A->rows; i++) {
        for (size_t j = 0; j < (size_t)A->cols; j++) {
          result->data[i][j] =
              A->data[i][j] - B->data[i][j]; ///< Вычитание элементов
        }
      }
    }
  }
    ...
    if (matrix->rows == 1) {
      ...
      for (size_t k = 0; k < (size_t)matrix->cols; k++) {
        ...
        if (minor) {
          ...
          det += sign * matrix->data[0][k] *
                 determinant(minor); // Вычисление миноров
...
char *converting_to_string(Matrix *matrix) {
  ...
  if (matrix) {
    ...
    size_t result_size = matrix->rows * matrix->cols * max_element_size +
                         1; ///< 12 байт на число (с запасом)
    ...
    if (result) {
      ...
      for (size_t i = 0; i < (size_t)matrix->rows && error_flag == 0; i++) {
        for (size_t j = 0; j < (size_t)matrix->cols && error_flag == 0; j++) {
          int written =
              snprintf(current_pos, result_size - (current_pos - result), "%d ",
                       matrix->data[i][j]);

          if (written < 0 ||
              written >= (int)(result_size - (current_pos - result))) {
            free(result);
